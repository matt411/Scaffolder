...
{
    public class IRepoController : Scaffolding
    {
...
{5}   
    ...
    {5}
        ...
        // POST: /{1}/Create
        [HttpPost]
        public ActionResult Create ([Bind(Include = {5}{1}Id,/*Include Object Properties to Bind*/{5})]{1} {2})
        {5}
            if (ModelState.IsValid) {5}
                unitOfWork.{1}Repository.Insert({2});
                unitOfWork.{1}Repository.Update({2});
                return RedirectToAction({5}Index{5});
            {6} else {5}
				return View();
			{6}
        {6}
        ...
        [HttpPost, ActionName({5}Delete{5})]
...
/////////////////////////////////
    ...
    {5}
        ...
        public {1}Repository(SkimosContext context)
        {5}
        ...
        {5}
            context.Entry({2}).State = System.Data.Entity.EntityState.Modified;
        {6}
